...
function PositionBadge({ position, regStatus }: { position: number | null | undefined; regStatus: string }) {
  if (position === 1) return <span className="text-[13px] font-bold text-amber">1st</span>;
  if (position === 2) return <span className="text-[13px] font-bold text-text-muted">2nd</span>;
  if (position === 3) return <span className="text-[13px] font-bold text-amber-600">3rd</span>;
  if (position != null) return <span className="text-[13px] font-semibold text-text-muted">{position}th</span>;
  if (regStatus === "failed") return <span className="text-[13px] font-semibold text-red">DNF</span>;
  return <span className="text-[13px] text-text-muted/70">—</span>;
}

// ── Reg status chip ───────────────────────────────────────────────────────────
const REG_CFG: Record<string, { label: string; color: string; bg: string }> = {
  pending:   { label: "PENDING",   color: "text-amber", bg: "bg-amber/10 border-amber/40" },
  accepted:  { label: "ACCEPTED",  color: "text-blue",   bg: "bg-blue/10 border-blue/40"    },
  verified:  { label: "VERIFIED",  color: "text-green",  bg: "bg-green/10 border-green/40"  },
  failed:    { label: "FAILED",    color: "text-red",    bg: "bg-red/10 border-red/40"      },
  rejected:  { label: "REJECTED",  color: "text-text-muted",   bg: "bg-white/8 border-white/15"   },
  cancelled: { label: "CANCELLED", color: "text-text-muted",   bg: "bg-white/8 border-white/15"   },
};
...
function StewardChip({ action }: { action: string }) {
  ...
  const color  = severe ? "text-red bg-red/10 border-red/40"
...
function ViolationStatusChip({ status }: { status: string }) {
  const cfg: Record<string, string> = {
    pending:   "text-amber bg-amber/10 border-amber/40",
    confirmed: "text-red bg-red/10 border-red/40",
    dismissed: "text-text-muted bg-white/8 border-white/12",
  };
  return (
  ...
  return (
    <div className="flex items-center gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <span key={i} className={`w-1.5 h-1.5 rounded-full ${i < level ? "bg-red" : "bg-white/10"}`} />
      ))}
    </div>
  );
}
...
function StatCell({ label, value, accent }: { label: string; value: string | number; accent?: boolean }) {
  return (
    <div className="bg-[#1e1e1e] rounded-xl px-3 py-3 border border-border text-center">
      <p className="text-[9px] font-semibold tracking-widest text-text-muted/70 uppercase mb-1">{label}</p>
      <p className={`text-[18px] font-bold ${accent ? "text-amber" : "text-text"}`}>{value}</p>
    </div>
  );
}
  ...
  return (
    <div className={`rounded-xl border px-4 py-3.5 flex items-center gap-4 ${statusGlow}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-3 text-[11px] text-text-muted flex-wrap mt-0.5">
          {rr?.raceDate && (
          ...
          {reg.laneNumber != null && (
            <span className="text-text-muted">Lane {reg.laneNumber}</span>
          )}
        </div>
      </div>
      {canReject && (
        <button
          ...
          className="text-[10px] font-bold px-2.5 py-1 rounded-full border border-red/40 text-red hover:bg-red/10 transition-colors duration-150 shrink-0"
        >
          Reject
        </button>
      )}
...
function RaceHistoryTab({ history, onRequestReject }: { history: HorseRegistrationEntry[]; onRequestReject: (id: string, label: string) => void }) {
  ...
  if (history.length === 0) {
    return (
      <div className="rounded-xl border border-border bg-white/2 px-5 py-12 text-center">
        <Trophy size={32} className="text-text-muted/50 mx-auto mb-3" />
        <p className="text-[14px] font-semibold text-text-muted mb-1">No races entered yet</p>
        <p className="text-[12px] text-text-muted/70 max-w-xs mx-auto leading-relaxed">
  ...
  return (
    <div className="space-y-5">
      ...
      {upcoming.length > 0 && (
        <div>
          <p className="text-[10px] font-bold tracking-widest text-amber/80 uppercase mb-2 flex items-center gap-1.5">
      ...
      {pastRaces.length > 0 ? (
        <div>
          {upcoming.length > 0 && (
            <p className="text-[10px] font-bold tracking-widest text-text-muted/70 uppercase mb-2">Race Results</p>
          )}
          <div className="space-y-2">
            {pastRaces.map((entry) => {
              const rr  = entry.raceRound;
        ...
        upcoming.length > 0 && (
          <div className="rounded-xl border border-border bg-white/2 px-4 py-6 text-center">
            <p className="text-[13px] text-text-muted/70">No race results yet — awaiting upcoming event.</p>
          </div>
        )
      )}
    </div>
      ...
      {violations.map((v) => (
        <div key={String(v._id)} className="bg-[#1e1e1e] rounded-xl border border-border px-4 py-4">
          <div className="flex items-start justify-between gap-4 mb-3">
            <div className="flex-1 min-w-0">
              <p className="text-[13px] font-bold text-text mb-0.5">
                {v.violationType?.violationName ?? "Unknown Violation"}
              </p>
              <div className="flex items-center gap-2 text-[11px] text-text-muted flex-wrap">
                <span className="capitalize text-text-muted bg-white/5 border border-border px-1.5 py-0.5 rounded text-[10px]">
          ...
          {v.description && (
            <p className="text-[12px] text-text-muted mb-3 leading-relaxed">{v.description}</p>
          )}
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {v.actualPenalty && (
              <span className="text-[10px] text-text-muted/70 bg-white/5 border border-border px-2 py-0.5 rounded">
                {v.actualPenalty}
              </span>
            )}
          </div>
...
export default function HorseProfile({ horseId, onClose }: HorseProfileProps) {
  ...
  const healthColor = !data ? "" :
    data.horse.healthStatus === "healthy"  ? "text-green bg-green/10 border-green/40" :
    data.horse.healthStatus === "injured"  ? "text-red bg-red/10 border-red/40" :
    "text-amber bg-amber/10 border-amber/40";
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
          <p className="text-[11px] font-bold tracking-[0.2em] text-text-muted/70 uppercase">Horse Profile</p>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
          ...
          {/* Error */}
          {!loading && error && (
            <div className="rounded-xl border border-error-border bg-error-bg px-4 py-4 flex items-center justify-between gap-3">
              ...
              <button
                type="button"
                onClick={load}
                className="shrink-0 flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-red/40 text-[11.5px] font-medium text-red hover:text-text hover:bg-red/20 transition-colors"
              >
                Retry
              </button>
            </div>
            ...
            return (
              <>
                ...
                <div className="bg-surface rounded-2xl border border-border p-5 flex items-center gap-5">
                  ...
                  <div className="flex-1 min-w-0">
                    <h2 className="text-[22px] font-bold text-text leading-tight font-serif">
                      {horse.horseName}
                    </h2>
                  ...
                  <div className="shrink-0 flex flex-col items-end gap-1.5">
                    ...
                    <span className="text-[10px] font-bold px-2.5 py-1 rounded-full border border-white/12 text-text-muted capitalize">
                      {horse.status}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1 p-1 bg-surface border border-border rounded-xl w-fit">
                  {([
                    ...
                    >
                      ...
                      <ChevronRight size={11} className="text-text-muted/70" />
                    </button>
                  ))}
                </div>

...
export default function OwnerProfilePage() {
    ...
    const load = async () => {
        setLoading(true);
        try {
            const res = await horseOwnerService.getMyProfile();
    ...
    const handleSave = async (payload: UpdateSelfProfilePayload) => {
        try {
            const res = await horseOwnerService.updateMyProfile(payload);
            if (res.code === 200) {
                setProfile(res.data);
    ...
    const handleLicenseReupload = async (file: File) => {
        try {
            const res = await horseOwnerService.updateMyLicense(file);
            if (res.code === 200) {
                setProfile(res.data);

const HorseOwnerHorseService = require('../../services/HorseOwnerHorseService');
...
class HorseController {
    ...
    async getMyHorses(req, res) {
        ...
        const response = await HorseOwnerHorseService.getOwnedHorses(req.userId, page, limit, search, sortBy, order);
        return res.status(response.code).json(response);
    }

    async getHorseProfile(req, res) {
        const { horseId } = req.params;
        const response = await HorseOwnerHorseService.getHorseProfile(req.userId, horseId);
        return res.status(response.code).json(response);
    }
    ...
    async updateHorseStatus(req, res) {
        ...
        const response = await HorseOwnerHorseService.updateHorseStatus(req.userId, horseId, status);
        return res.status(response.code).json(response);
    }
    ...
    async updateHorseHealthStatus(req, res) {
        ...
        const response = await HorseOwnerHorseService.updateHorseHealthStatus(req.userId, horseId, healthStatus);
        return res.status(response.code).json(response);
    }

    async getRaceEligibilityMetadata(req, res) {
        const { ruleId } = req.query;
        const response = await HorseOwnerHorseService.getRaceEligibilityMetadata(ruleId);
        return res.status(response.code).json(response);
    }
}

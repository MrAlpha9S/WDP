...
class PayoutService {
    ...
    // Bᵢ = sum(prediction.rewardPoints) for that horse while status='pending'.
    // ─────────────────────────────────────────────────────────────────────────

    async getPoolSnapshot(raceRoundId, methodType = 'race_winner', customTakeout = null) {
        try {
            ...
            // Sum stake (rewardPoints) per registrationId → Bᵢ
            const stakeByReg = {};
